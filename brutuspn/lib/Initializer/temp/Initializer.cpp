...
vector<double> Initializer::generate(int N, string config, vector<string> par) {
  ...
  if(N == 20) {
    if(config == "solar_system_halley") return get_solar_system_halley( N, atoi(par[0].c_str()), atoi(par[1].c_str()), atoi(par[2].c_str()), atof(par[3].c_str()) );
  }
...
vector<double> Initializer::get_solar_system_halley(int N, int seed, int pivot, int index, double f_error) {
  vector<double> data;
  ...
  data.push_back(-2.046011952809097E+01*(1+dx));
  data.push_back(2.513980121805306E+01*(1+dy));
  data.push_back(-9.771054962044406E+00*(1+dz));
  data.push_back(-6.631852646093806E-05*(1+dvx));
  data.push_back(9.404758449357039E-04*(1+dvy));
  data.push_back(-1.740327105577856E-04*(1+dvz));
